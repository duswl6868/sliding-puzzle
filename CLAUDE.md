...
| 그리드 | 일반 모드 홈 카드는 **3×3 / 스테이지 퍼즐(1~30탄)**. 게임 화면 `N × N ▼` 모달로 **3·4·5 디폴트 + 클리어한 사이즈의 +1 해금** (5 클리어 → 6 열림, 8 클리어 → 9 열림). 무한 모드는 3부터 ∞ 진행 |
...
여러 개의 풀스크린 `.screen`, `.active` 클래스로 전환.

**규칙 — 새 페이지 추가 시 기본 탑재**:
1. `show()`에 등록 (이 screen도 `.active` 토글에 포함)
2. **좌측 엣지 스와이프 뒤로가기 등록** — `attachEdgeSwipeBack(screen, onBack)` 호출. iOS 표준 제스처와 일관. 게임 진행 중인 `playScreen`처럼 특수 조건이 필요한 경우만 onBack 안에 가드.
3. 좌상단 ← 뒤로 버튼 (icon-btn + chevron-left svg) — 클릭 시 동일 onBack.
...
- 난이도 카드 4장 (글라스 카드):
  ...
  2. **스테이지 퍼즐** — 노란 별 썸네일. 클릭 시 `#stagesScreen` 진입 (1~30탄 4열 그리드, 1탄만 활성)
...
### Stages (`#stagesScreen`)
- 진입: 홈 카드 "스테이지 퍼즐" 클릭. 좌상단 ← 뒤로 + 좌측 엣지 스와이프
- 그리드: 4열 정사각 카드, 1~30탄 (`TOTAL_STAGES = 30`). 한 줄에 4개씩 — `repeat(4, 1fr)`, gap `clamp(10, 2.6vw, 14)`
- 카드 상태:
  - **활성**(1탄만): 글라스 톤 + 블루 숫자, 클릭 시 `pickStage(n)` → `startGame(4)` (잠정, 후속에 스테이지별 난이도 분기)
  - **잠금**(2~30탄): 회색 톤 + **노란 자물쇠**(`#e5b53a`, `lock-icon`), `disabled`, `pointer-events` 자연 차단
- 후속(현재 미구현): 스테이지 클리어 시 다음 잠금 해제, 스테이지별 난이도/사이즈 분기, LS에 진행도 저장

### Stats (`#statsScreen`)
- 진입: 홈 하단 `home-stats` **native `<button>`** 클릭 (스크롤 직후에도 click이 안정적으로 발화. div+role 합성 click 누락 문제 회피). 좌상단 ← 뒤로 / **좌측 엣지 스와이프(< 28px → 80px, < 60px Y, < 800ms)** 도 동일 동작 (`attachEdgeSwipeBack`)
